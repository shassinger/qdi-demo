...
pkill -f "http.server ${QDI_WEB_PORT}" || true
...
set_codespace_port_visibility() {
    ...
    if ! command -v gh >/dev/null 2>&1; then
        echo "GitHub CLI not found; set ports ${QDI_WEB_PORT} and ${QDI_API_PORT} public manually if needed."
        return 0
    fi

    echo "Setting Codespaces ports ${QDI_WEB_PORT} and ${QDI_API_PORT} to public visibility..."
    if GH_TOKEN="${GITHUB_TOKEN:-}" gh codespace ports visibility "${QDI_WEB_PORT}:public" "${QDI_API_PORT}:public" -c "$CODESPACE_NAME"; then
        echo "Codespaces port visibility set to public."
    else
        echo "Unable to set port visibility automatically; set ports ${QDI_WEB_PORT} and ${QDI_API_PORT} public in the Ports tab."
    fi
}
...
QDI_API_HOST="$QDI_API_HOST" QDI_API_PORT="$QDI_API_PORT" nohup .venv/bin/python -u qdi-core/python/qdi_demo_server.py < /dev/null > server.log 2>&1 &
API_PID=$!
...
nohup .venv/bin/python -u -m http.server --bind "$QDI_WEB_HOST" --directory qdi-core/python "$QDI_WEB_PORT" < /dev/null > web.log 2>&1 &
WEB_PID=$!
echo "$WEB_PID" > web.pid

wait_for_url "QDI Backend API" "http://127.0.0.1:${QDI_API_PORT}/health" "$API_PID" "server.log"
wait_for_url "QDI Web Console" "http://127.0.0.1:${QDI_WEB_PORT}/index.html" "$WEB_PID" "web.log"
set_codespace_port_visibility
