...
class NativeQdiClient:
    """Server-side Python wrapper for the local QDI C-ABI device."""
...
class QdiClient:
    """Application-facing QDI client that communicates over HTTP(S)."""
    ...
    def __init__(self, base_url=None, token=None, timeout=30.0, client=None):
        try:
            ...
            raise RuntimeError(
                "The HTTP QDI client requires httpx. Install the demo dependencies first."
            ) from exc

        self.base_url = (
            base_url or os.environ.get("QDI_API_URL", "http://127.0.0.1:8000")
        ).rstrip("/") + "/"
        self.token = token
    ...
    def _request(self, method, path, **kwargs):
        ...
        if response.is_error:
            ...
            error_codes = {401: 2, 404: 4}
            raise QDIError(error_codes.get(response.status_code, 99), str(detail))
        return response.json()

    def discover(self) -> dict:
        return self._request("GET", "/qdi/v1/devices/mock/discover")
    ...
    def send(self, task_payload: bytes, task_type: str, shots: int = 100) -> str:
        response = self._request(
            ...
            json={
                "task_payload": task_payload.decode("utf-8"),
                "task_type": task_type,
                "shots": shots,
            },
        )
    ...
    def monitor(self, task_id: str) -> tuple[int, dict]:
        ...
        return statuses.get(response["status"], 99), response.get("advisory_metadata", {})
